...
--- That means one customer have multiple state that means we can not use query to count no of customer by state
...
--There are Bottom category also on which Olsit have to improve 
...
-- Which product category gets most time to Deliver
...
--- #. There is null value in product category also whaich gives pretty revenu to company
